...
class TypeScriptExtension extends PicteusExtension
{
  ...
  private async handleAskForSomething(communicator: Communicator, parameters: Record<string, any>): Promise<void>
  {
    ...
    try
    {
        ...
        form:
          {
            ...
            dialogContent: {
              ...
              details: `This dialog box has been dynamically generated from the extension source code, based on the previous dialog box answer with value '${parameters.age}'.`,
              icon: {
      ...
      communicator.sendLog(`You picked the '${userParameters.favoriteColor}' color and ${userParameters.likeChocolate === true ? "declared that you like chocolate" : "did not mention that you liked chocolate"}`, "info");
    }
    catch (error)
    {
  ...
  private async handleUi(communicator: Communicator, parameters: Record<string, any>): Promise<void>
  {
    const anchor: string = parameters["anchor"];

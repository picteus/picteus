...
class C2paExtension extends PicteusExtension
{
  ...
  {
    if (commandId === "readMetadata")
    {
      await this.readMetadata(communicator, imageIds);
    }
  }

  private async readMetadata(communicator: Communicator, imageIds: string[]): Promise<void>
  {

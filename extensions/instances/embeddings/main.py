...
class Embeddings(PicteusExtension):
    ...
    async def on_image_created(self, communicator: Communicator, image_id: str) -> None:
        await self._compute_image_embeddings(communicator, image_id, self.clip_enabled, self.dino_enabled)
    ...
    async def on_process_command(self, communicator: Communicator, command_id: str,
        ...
        if command_id == "compute":
            ...
            clip_enabled: bool = parameters["clipEnabled"]
            dino_enabled: bool = parameters["dinoEnabled"]
            search_result = await self.run_in_executor(
                lambda: self.get_image_api().image_search_ids(
    ...
    async def on_compute_text_embeddings(self, communicator: Communicator, text: str) -> list[float]:
        result = await self.run_in_executor(lambda: self._compute_clip_text_embedding(communicator, text))
        await self.run_in_executor(self._clean_up_torch)
        return result
    ...
    async def _compute_image_embeddings(self, communicator: Communicator, image_id: str, clip_enabled: bool,
        ...
        async with self._get_semaphore():
            image: bytearray = await self.run_in_executor(
                lambda: self.get_image_api().image_download(id=image_id, format=ImageFormat.PNG, width=256,
                                                           height=256, resize_render=ImageResizeRender.OUTBOX,
                                                           strip_metadata=True))
            pil_image: ImageFile = Image.open(io.BytesIO(image))

            await self.run_in_executor(lambda: self._ensure_models(clip_enabled, dino_enabled))

            try:
                existing_embeddings_result = await self.run_in_executor(
                    lambda: self.get_image_api().image_get_embeddings(id=image_id,
            ...
            if len(embeddings) > 0:
                await self.run_in_executor(
                    lambda: self.get_image_api().image_set_embeddings(id=image_id, extension_id=self.extension_id,
    ...
    async def _reset_embeddings(self, communicator: Communicator) -> None:
        ...
        extension_settings = await self.run_in_executor(
            lambda: self.get_extension_api().extension_reset_settings(id=self.extension_id))
        self._setup(extension_settings.value)
        search_result = await self.run_in_executor(
            lambda: self.get_image_api().image_search_ids(
                search_parameters=SearchParameters(filter=SearchFilter())))
        ids: List[str] = search_result.items
        for image_id in ids:
            await self.run_in_executor(
                lambda: self.get_image_api().image_set_embeddings(id=image_id, extension_id=self.extension_id,
                                                                  image_embedding=[]))

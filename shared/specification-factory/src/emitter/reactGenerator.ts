...
function computeTypeScriptImports(spec: GrammarSpec): string[]
{
  ...
  // We include all standalone models
  for (const model of spec.models)
  {
    importNames.add(model.name);
  }
...
function generateRowSlotsLayoutBody(): string
{
  return [
    ...
    `        <Box key={slotIndex} style={{ flex: slot.width !== undefined ? \`0 1 \${slot.width}%\` : 1, ...CONSTRAINED_STYLE }}>`,
...
function generateTableLayoutBody(): string
{
  return [
    ...
    `  const columnsList = element.columns ?? Array.from({ length: columnCount }, () => ({ width: undefined, widthMode: undefined, align: undefined, header: undefined }));`,
    ``,
    `  function renderCellContent(column: TableColumn | undefined, contentNode: ReactNode): ReactNode`,
    `  {`,
    `    const isMax = column?.width !== undefined && column.widthMode === TableColumnWidthMode.maximum;`,
    `    if (isMax)`,
    `    {`,
    `      return (`,
    `        <Box style={{ width: "max-content", maxWidth: \`\${column.width}cqw\`, display: "inline-block", minWidth: 0, ...TEXT_WRAP_STYLE }}>`,
    `          {contentNode}`,
    `        </Box>`,
    `      );`,
    `    }`,
    ...
    ``,
    `  return (`,
    `    <Box className={className} style={{ ...FULL_WIDTH_CONSTRAINED_STYLE, containerType: "inline-size", ...style }}>`,
    `      <Table striped={element.isStriped} highlightOnHover withColumnBorders={element.withColumnSeparators} withRowBorders={element.withRowSeparators} style={FULL_WIDTH_CONSTRAINED_STYLE}>`,
    `        <colgroup>`,
    `          {columnsList.map((column, columnIndex) => (`,
    `            <col key={columnIndex} style={{ width: column.width !== undefined ? (column.widthMode === TableColumnWidthMode.maximum ? "1%" : \`\${column.width}%\`) : undefined }}/>`,
    `          ))}`,
    `        </colgroup>`,
    `        {element.columns?.some((column) => Boolean(column.header)) && (`,
    `          <Table.Thead>`,
    `            <Table.Tr>`,
    `              {element.columns.map((column, columnIndex) => (`,
    `                <Table.Th key={columnIndex} style={{ textAlign: column.align ?? "left", width: column.width !== undefined ? (column.widthMode === TableColumnWidthMode.maximum ? "1%" : \`\${column.width}%\`) : undefined, minWidth: 0, ...TEXT_WRAP_STYLE }}>`,
    `                  {renderCellContent(column, column.header ?? "")}`,
    `                </Table.Th>`,
    `              ))}`,
    `            </Table.Tr>`,
    `          </Table.Thead>`,
    ...
    `                <Table.Td key={cellIndex} style={{ textAlign: columnsList[cellIndex]?.align ?? "left", width: columnsList[cellIndex]?.width !== undefined ? (columnsList[cellIndex]?.widthMode === TableColumnWidthMode.maximum ? "1%" : \`\${columnsList[cellIndex]?.width}%\`) : undefined, minWidth: 0, ...TEXT_WRAP_STYLE }}>`,
    `                  {renderCellContent(columnsList[cellIndex], <${UI_ELEMENT_VIEW_NAME} element={cell} onAction={onAction}/>)}`,
    `                </Table.Td>`,
    `              ))}`,
    `            </Table.Tr>`,
    `          ))}`,

...
The `type` attribute (`ImageFeatureType`) declares the semantic intent and purpose of the feature. Picteus recognizes eight distinct semantic types:
...
|:---|:---|:---|:---|
| **`caption`** | `string` | `integer`, `float`, `boolean`, `json`, `yaml`, `xml`, `markdown`, `html`, `binary`, `ui` | **Plain text only**. Captions are designed as concise single-line textual descriptions. Formatting markup, structured objects, numeric values, and binary assets are strictly rejected. |
...
| **`recipe`** | `json` | `string`, `integer`, `float`, `boolean`, `yaml`, `xml`, `markdown`, `html`, `binary`, `ui` | **Structured JSON only**. Generative recipes represent machine-executable workflows. Must be JSON and must strictly comply with the `GenerationRecipe` schema. Plain text or non-JSON formats are rejected. |
...
- **Recipe schema verification**: when `type === "recipe"`, the parsed JSON object is additionally validated against the `GenerationRecipe` class schema (verifying `prompt`, `modelTags`, `schemaVersion`, `aspectRatio`, etc.). If validation fails, the parameter checker throws an error.
...
```json
{
  ...
  "format": "json",
  "name": "flux_workflow",
  "value": "{\"schemaVersion\":1,\"software\":\"ComfyUI\",\"modelTags\":[\"flux-1-dev\",\"fp8\"],\"prompt\":{\"prompt\":\"A detailed photographic portrait of an elderly watchmaker in his workshop surrounded by antique gears and clocks\",\"negativePrompt\":\"blurry, low quality, distorted\"},\"aspectRatio\":1.333}"
}
```
...
#### 2. Get image features for extension: `GET /image/{id}/getFeatures`
...
#### 3. Get all image features: `GET /image/{id}/getAllFeatures`
...
#### 4. Get all image recipes: `GET /image/{id}/getAllRecipes`
...
#### 5. Get all repository feature names: `GET /repository/featureNames`

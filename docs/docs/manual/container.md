...
- **Native OS dialogs & file explorers**: desktop features relying on native operating system prompts — such as "reveal in Finder / File Explorer" or native directory picker dialogs — are unavailable and fallback to standard browser-based file downloads or manual path inputs ;
- **No Chrome extension host runtime**: the native Electron application includes a dedicated runtime for packaged Chromium extensions. This capability is not embedded in the containerized web UI ;

...
const updatePythonOpenApiPackageConfiguration = (directoryPath) =>
{
  const pyProjectFilePath = path.join(directoryPath, "pyproject.toml");
  if (fs.existsSync(pyProjectFilePath) === true)
  {
    const fileContent = fs.readFileSync(pyProjectFilePath, { encoding: "utf8" });
    // We update the license specification to comply with PEP 639 and avoid setuptools deprecation warnings
    const updatedFileContent = fileContent.replace(/license\s*=\s*\{\s*text\s*=\s*["']([^"']+)["']\s*}/g, "license = \"$1\"");
    if (updatedFileContent !== fileContent)
    {
      fs.writeFileSync(pyProjectFilePath, updatedFileContent, { encoding: "utf8" });
    }
  }
  // We remove the legacy setup.py file generated by OpenAPI Generator to avoid duplicate metadata warnings from setuptools
  const setupPyFilePath = path.join(directoryPath, "setup.py");
  if (fs.existsSync(setupPyFilePath) === true)
  {
    fs.rmSync(setupPyFilePath, { force: true });
  }
};
...
export const packagePythonOpenApiClient = () =>
{
  ...
  updatePythonOpenApiPackageConfiguration(directoryPath);

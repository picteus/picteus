...
export const FiltersBar = forwardRef<FiltersBarRef, FiltersBarType>(({ initialFilterOrCollectionId, onFilterOrCollectionId, onClearAll, children }, ref) => {
  ...
  return (
    <Flex gap={10} align="flex-end">
            ...
            <Input
              ...
              rightSection={
                <Flex gap={4} wrap="nowrap" align="center">
                  {searchText || filters ? (
                    <ActionIcon size="sm" variant="transparent" c="dimmed" onClick={handleOnClearAll}>
          ...
          <Tabs defaultValue="general">
            <Group justify="space-between" align="center" pr="sm">
              <Tabs.List flex={1}>
                <Tabs.Tab
                  value="general"
                  leftSection={<IconAdjustmentsHorizontal size={edge} color="var(--mantine-color-blue-filled)" />}
                >
                  {t("field.essentials")}
                </Tabs.Tab>
                <Tabs.Tab
                ...
                <Tabs.Tab
                  value="properties"
                  leftSection={<IconListDetails size={edge} color="var(--mantine-color-green-filled)" />}
                >
                  {t("field.properties")}
                </Tabs.Tab>
              </Tabs.List>
      ...
      <Button.Group>
        <Menu shadow="md" width={160} position="bottom" trigger="click-hover" opened={sortingMenuOpened} onChange={setSortingMenuOpened}>
          <Menu.Target>
            <Button variant="default" w={160} leftSection={filters?.sortBy ? SORT_ICONS[filters.sortBy] : null}
          ...
          <Menu.Dropdown style={{ maxHeight: "75%", overflowY: "auto" }}>
            {FiltersService.sortByOptions.map((option) => (
              <Menu.Item key={option.value} leftSection={filters?.sortBy ? SORT_ICONS[option.value] : null}
